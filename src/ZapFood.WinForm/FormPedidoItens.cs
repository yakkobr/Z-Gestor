...
namespace ZapFood.WinForm
{
    public partial class FormPedidoItens : Form
    {
        ...
        private void LoadObject()
        {
            ...
            var Pedido = _pedido.subMeioMeios;

            if (Pedido.Length > 0)
            {
                List<teste> a = new List<teste>();
                var texto = string.Empty;
                var NomeDaMetade = string.Empty;
                var num = 0;
                for (int e = 0; e < Pedido.Length; e++)
                {
                    var PedidoCompl = Pedido[e].complementosMeioMeio;

                    for (int e2 = 0; e2 < PedidoCompl.Length; e2++)
                    {

                        if (PedidoCompl[e2].NomeDaMetade != NomeDaMetade)
                        {
                            if (e == 0)
                            {
                                texto = $"{PedidoCompl[e2].NomeDaMetade}";
                                texto += $"\n + {PedidoCompl[e2].Quantidade} x {PedidoCompl[e2].Nome} R$ {PedidoCompl[e2].Valor}";

                                NomeDaMetade = PedidoCompl[e2].NomeDaMetade;
                                //num = PedidoCompl.NumDaPizza;
                            }
                            else
                            {
                                a.Add(new teste
                ...
                for (int e = 0; e < Pedido.Length; e++)
                {
                    ...
                    for (int e2 = 0; e2 < PedidoCompl.Length; e2++)
                    {
                        ...
                        valortotal = valortotal + (PedidoCompl[e2].Valor * PedidoCompl[e2].Quantidade);
                    }
                }
